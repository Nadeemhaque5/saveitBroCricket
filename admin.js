...
class CricketAdmin {
    ...
    setupEventListeners() {
        ...
        console.log('Add channel form found:', !!addChannelForm);
        if (addChannelForm) {
            ...
            console.error('Add channel form not found in DOM');
        }
        ...
        // Thumbnail upload functionality for add channel form
        const logoUploadBtn = document.getElementById('upload-logo-btn');
        const logoFileInput = document.getElementById('logo-file');
        const logoFileName = document.getElementById('file-name');
        ...
        const thumbnailOptions = document.querySelectorAll('input[name="thumbnailOption"]');
        ...
        // File selection
        if (logoFileInput && logoFileName && logoPreview && logoPreviewContainer) {
        ...
        // URL preview button
        if (previewLogoBtn && logoUrlInput && logoPreview && logoPreviewContainer) {
        ...
        // Clear logo button
        if (clearLogoBtn && logoPreviewContainer && logoFileInput && logoUrlInput && logoFileName) {
        ...
        // Thumbnail option radio buttons
        if (thumbnailOptions.length > 0) {
            thumbnailOptions.forEach(option => {
                option.addEventListener('change', () => {
                    const urlOption = document.querySelector('.upload-option:has(input[value="url"])');
                    const uploadOption = document.querySelector('.upload-option:has(input[value="upload"])');
                    
                    if (option.value === 'url') {
                        urlOption.style.opacity = '1';
                        urlOption.style.pointerEvents = 'auto';
                        uploadOption.style.opacity = '0.5';
                        uploadOption.style.pointerEvents = 'none';
                    } else {
                        uploadOption.style.opacity = '1';
                        uploadOption.style.pointerEvents = 'auto';
                        urlOption.style.opacity = '0.5';
                        urlOption.style.pointerEvents = 'none';
                    }
                });
            });
        }

        // Edit modal thumbnail upload functionality
        const editLogoUploadBtn = document.getElementById('edit-upload-logo-btn');
        const editLogoFileInput = document.getElementById('edit-logo-file');
        const editLogoFileName = document.getElementById('edit-file-name');
        ...
        const editPreviewLogoBtn = document.getElementById('edit-preview-logo');
        const editClearLogoBtn = document.getElementById('edit-clear-logo');
        const editLogoUrlInput = document.getElementById('edit-logo-url');
        const editThumbnailOptions = document.querySelectorAll('input[name="editThumbnailOption"]');

        // Edit upload button click
        if (editLogoUploadBtn && editLogoFileInput) {
            editLogoUploadBtn.addEventListener('click', () => {
                editLogoFileInput.click();
            });
        }

        // Edit file selection
        if (editLogoFileInput && editLogoFileName && editLogoPreview && editLogoPreviewContainer) {
            editLogoFileInput.addEventListener('change', (e) => {
                ...
                if (file) {
                    ...
                    editLogoFileName.textContent = file.name;
        ...
        // Edit URL preview button
        if (editPreviewLogoBtn && editLogoUrlInput && editLogoPreview && editLogoPreviewContainer) {
            editPreviewLogoBtn.addEventListener('click', () => {
                const url = editLogoUrlInput.value.trim();
                if (!url) {
                    this.showToast('Please enter a URL first', 'warning');
                    return;
                }
        ...
        // Edit clear logo button
        if (editClearLogoBtn && editLogoPreviewContainer && editLogoFileInput && editLogoUrlInput && editLogoFileName) {
            editClearLogoBtn.addEventListener('click', () => {
                editLogoPreviewContainer.style.display = 'none';
                editLogoFileInput.value = '';
                editLogoUrlInput.value = '';
                editLogoFileName.textContent = '';
            });
        }

        // Edit thumbnail option radio buttons
        if (editThumbnailOptions.length > 0) {
            editThumbnailOptions.forEach(option => {
                option.addEventListener('change', () => {
                    const urlOption = document.querySelector('.upload-option:has(input[value="url"])');
                    const uploadOption = document.querySelector('.upload-option:has(input[value="upload"])');
    ...
    updateActivityList() {
        ...
        if (!form) {
            this.showToast('Add channel form not found', 'error');
            return;
        }
        ...
        try {
            ...
            const thumbnailData = await this.processThumbnailData(formData);
            if (thumbnailData) {
                channelData.thumbnail = thumbnailData;
            }
        ...
        const editLogoPreview = document.getElementById('edit-logo-preview');
        const editLogoFileName = document.getElementById('edit-file-name');
        const editUrlOption = document.querySelector('input[name="editThumbnailOption"][value="url"]');
        const editUploadOption = document.querySelector('input[name="editThumbnailOption"][value="upload"]');

        // Clear previous thumbnail data
        if (editLogoUrlInput) editLogoUrlInput.value = '';
        if (editLogoFileName) editLogoFileName.textContent = '';
        if (editLogoPreviewContainer) editLogoPreviewContainer.style.display = 'none';
        
        // Set thumbnail data if exists
        if (channel.thumbnail) {
            if (channel.thumbnail.type === 'url') {
                if (editUrlOption) editUrlOption.checked = true;
                if (editLogoUrlInput) editLogoUrlInput.value = channel.thumbnail.value;
                if (editLogoPreview && editLogoPreviewContainer) {
                    editLogoPreview.src = channel.thumbnail.value;
                    editLogoPreviewContainer.style.display = 'block';
                }
            } else if (channel.thumbnail.type === 'upload') {
                if (editUploadOption) editUploadOption.checked = true;
                if (editLogoFileName) editLogoFileName.textContent = channel.thumbnail.fileName || 'Uploaded image';
                if (editLogoPreview && editLogoPreviewContainer) {
                    editLogoPreview.src = channel.thumbnail.value;
                    editLogoPreviewContainer.style.display = 'block';
                }
        ...
        // Trigger radio button change to update UI
        const checkedOption = document.querySelector('input[name="editThumbnailOption"]:checked');
        if (checkedOption) {
        ...
        try {
            // Process thumbnail data with edit form
            const editFormData = new FormData();
            const editThumbnailOption = document.querySelector('input[name="editThumbnailOption"]:checked')?.value;
            const editLogoUrl = document.getElementById('edit-logo-url')?.value;
            const editLogoFile = document.getElementById('edit-logo-file')?.files[0];
            
            editFormData.set('thumbnailOption', editThumbnailOption || 'url');
            if (editLogoUrl) editFormData.set('logoUrl', editLogoUrl);
            if (editLogoFile) editFormData.set('logoFile', editLogoFile);
            
            const thumbnailData = await this.processThumbnailData(editFormData);
            if (thumbnailData) {
                channelData.thumbnail = thumbnailData;
            }
    ...
    resetData() {
        ...
        try {
            ...
            // Also trigger a more specific event
            const event = new CustomEvent('cricketChannelsChanged', {
            ...
            this.showToast('Failed to save channels', 'error');
        }
    }
    ...
    // Helper method to handle thumbnail data
    async processThumbnailData(formData) {
        const thumbnailOption = formData.get('thumbnailOption');
        let thumbnailData = null;

        if (thumbnailOption === 'url') {
            ...
            if (logoUrl) {
                ...
                // Test if image loads
                try {
                    await this.testImageLoad(logoUrl);
                    thumbnailData = {
                        type: 'url',
                        value: logoUrl
                    };
        ...
        } else if (thumbnailOption === 'upload') {
